...
const report = {
  startedAt: new Date().toISOString(), baseURL, browser: 'isolated Chromium',
  scope: 'Actual public Privy configuration and wallet chooser open, dismiss, reopen. No wallet selected; no injected provider, login, link, signature, or trade. Only anonymous Privy analytics POSTs are allowed.',
  checks: [], publicAppConfigs: [], allowedAnalytics: [], blockedWrites: [],
  pageErrors: [], consoleErrors: [], screenshots: [], sourceHashes: {},
};
await mkdir(output, { recursive: true });
for (const file of ['src/components/wallet-provider.tsx', 'src/components/exit-workspace.tsx', 'scripts/privy-smoke.mjs']) {
  ...
  responseReads.push((async () => {
    try {
      ...
      report.publicAppConfigs.push({
        capturedAt: new Date().toISOString(), url: `${url.origin}${url.pathname}`,
        status: response.status(), id: app.id, name: app.name,
        wallet_auth: app.wallet_auth, allowed_domains: app.allowed_domains,
        logo_url: app.logo_url,
      });
    } catch { report.publicAppConfigs.push({ url: `${url.origin}${url.pathname}`, status: response.status(), parseError: true }); }
  })());
});
...
async function screenshot(suffix) {
  ...
  report.screenshots.push(`docs/qa/${file}`);
}
const nativeWallet = page.locator('dialog').filter({ has: page.getByText('Your wallet & positions', { exact: true }) });
...
try {
  ...
  await check('Public app config loads with wallet authentication enabled', async () => {
    await expect.poll(() => report.publicAppConfigs.some((app) => app.status === 200 && app.wallet_auth === true)).toBe(true);
    assert.equal(await page.evaluate(() => Boolean(window.ethereum || window.solana)), false, 'A wallet provider was unexpectedly injected.');
    // The public configuration response precedes connector initialization.
    await page.waitForTimeout(1500);
  });
  await check('Inner Connect opens an actionable Privy chooser outside the native modal', async () => {
    await openChooser();
    report.nativeDialogsAtChooser = await page.locator('dialog:modal').count();
    await screenshot('chooser-open');
    assert.equal(report.nativeDialogsAtChooser, 0, 'Closeout native modal still owns the top layer and blocks Privy.');
  ...
  await check('No real wallet, account history, auth action, or order was created', async () => {
    ...
    const actions = report.blockedWrites.filter(({ path: pathname }) => !pathname.startsWith('/cdn-cgi/'));
  ...
  console.log(JSON.stringify({ passed: report.passed, publicAppConfigs: report.publicAppConfigs, report: `docs/qa/${stem}.json`, failure: report.failure }, null, 2));
}
