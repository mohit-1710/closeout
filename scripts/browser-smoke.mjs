...
const guardCases = [
  ...
  [
    "POST",
    "https://auth.privy.io/api/v1/analytics_events?session=fixture",
    false,
    "unexpected-write",
  ],
  ...
  [
    "POST",
    "https://auth.privy.io/cdn-cgi/challenge-platform/h/g/jsd/oneshot/fixture",
    false,
    "cdn-background",
  ],
...
for (const [method, target, auth, expected] of guardCases)
  assert.equal(requestClass(method, target, auth), expected);
...
await context.route("**/*", async (route) => {
  ...
  if (url.pathname === "/api/markets") {
    ...
    return route.fulfill({
      status: marketResponse === "error" ? 503 : 200,
      contentType: "application/json",
      body: JSON.stringify(
        marketResponse === "error"
          ? { error: "Smoke fixture: live market source unavailable." }
          : { markets: [] },
      ),
    });
  }
  if (url.hostname === "polymarket.com" && url.pathname === "/api/geoblock") {
    ...
    return route.fulfill({
      status: 503,
      contentType: "application/json",
      body: JSON.stringify({ error: "Smoke fixture: eligibility unavailable." }),
    });
  }
...
async function screenshot(name) {
  ...
  report.screenshots.push(`.artifacts/qa/${name}`);
}
async function noOverflow() {
  ...
  assert.ok(
    sizes.document <= sizes.width + 1 && sizes.body <= sizes.width + 1,
    `Horizontal overflow: ${JSON.stringify(sizes)}`,
  );
}
const ticket = page.getByRole("complementary", { name: "Exit ticket" });
const quoteRow = (label) =>
  ticket.locator(".co-breakdown > div").filter({ has: page.getByText(label, { exact: true }) });
try {
  await page.goto(baseURL, { waitUntil: "domcontentloaded" });
  await check(
    "Live GET failure stays live, with no synthetic market or balance fallback",
    async () => {
      ...
      await expect(page.getByRole("button", { name: "Example", exact: true })).toHaveAttribute(
        "aria-pressed",
        "false",
      );
      ...
      await expect(
        page.getByRole("button", { name: "Try example mode", exact: true }),
      ).toBeVisible();
      await noOverflow();
  ...
  await check(
    "Explicit Example mode exposes fictional data and a working market search",
    async () => {
      await page.getByRole("button", { name: "Try example mode", exact: true }).click();
      await expect(
        page.getByText("Example mode uses synthetic markets, balances and fills. No funds move.", {
          exact: true,
        }),
      ).toBeVisible();
  ...
  await check("Fraction controls and strict positive floor validation work", async () => {
    ...
    await expect(
      page.getByText("Enter a price greater than 0 and below 1 pUSD.", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Review simulated exit", exact: true }),
    ).toBeDisabled();
  ...
  await check("FOK blocks insufficient full depth; keyboard returns to FAK", async () => {
    ...
    await page
      .getByRole("radio", { name: "All or nothing Fill all shares or none" })
      .press("ArrowLeft");
    await expect(
      page.getByRole("radio", { name: "Sell available Cancel any remainder" }),
  ...
  await check(
    "Wallet dialog opens, Escape closes, focus returns; no connection is requested",
    async () => {
  ...
  await check(
    "Review supports back/Escape, then explicitly simulates one partial exit",
    async () => {
      ...
      await expect(
        page.locator(".co-order-row").getByText("Partially settled", { exact: true }),
      ).toBeVisible();
      await expect(
        page.locator(".co-order-row").getByText("Simulated", { exact: true }),
      ).toBeVisible();
      ...
      await expect(
        page.locator(".co-order-row").getByRole("button", { name: "Cancel", exact: true }),
      ).toHaveCount(0);
      await screenshot("closeout-activity-desktop.png");
    },
  );
  await check(
    "Activity export preserves example attribution and empty real transaction hashes",
    async () => {
  ...
  await check(
    "Mobile 375px Exit / Markets / Activity remain usable with no horizontal overflow",
    async () => {
  ...
  await check(
    "No uncaught browser errors or auth/link/order/financial write attempts occurred",
    async () => {
