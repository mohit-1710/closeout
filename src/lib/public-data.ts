...
export function isTokenId(value: unknown): value is string {
  return (
    typeof value === "string" && /^(?:0|[1-9]\d{0,77})$/.test(value) && BigInt(value) <= UINT256_MAX
  );
}
...
function decimal(
  value: unknown,
  label: string,
  options: { positive?: boolean; max?: string; stringOnly?: boolean } = {},
): string {
...
function arrayField(value: unknown, label: string): unknown[] {
  if (typeof value === "string") {
    if (value.length > 100_000)
      throw new PublicDataError(`The provider returned oversized ${label}.`);
    try {
      value = JSON.parse(value);
    } catch {
      throw new PublicDataError(`The provider returned invalid ${label}.`);
    }
  }
...
export function parsePublicJson(body: string): unknown {
  try {
    return JSON.parse(body, (_key, value, context?: { source?: string }) => {
      ...
      if (!context?.source)
        throw new PublicDataError("This server cannot preserve provider decimal precision.");
      ...
      if (!d.isFinite() || d.e > 100 || d.e < -100)
        throw new PublicDataError("The provider returned an out-of-range numeric value.");
      return d.toFixed();
    });
  } catch (error) {
...
export function normalizeMarket(value: unknown): Market | null {
  ...
  if (
    ...
    return null;
  if (!isConditionId(raw.conditionId))
    throw new PublicDataError("The provider returned a market without a valid condition ID.");
  ...
  if (
    ...
    !tokens.every(isTokenId)
  ) {
    throw new PublicDataError("Market outcomes could not be bound to unique token IDs.");
  }
  const prices =
    raw.outcomePrices == null
      ? labels.map(() => null)
      : arrayField(raw.outcomePrices, "outcome prices");
  if (prices.length !== labels.length)
    throw new PublicDataError("Market outcome prices do not match the token count.");
  ...
  if (endDate !== null && !Number.isFinite(Date.parse(endDate)))
    throw new PublicDataError("The provider returned an invalid market end date.");
  // Optional discovery metrics are display-only, not executable prices or balances.
  const volume24h = Number(
    decimal(raw.volume24hr ?? raw.volume24hrClob ?? "0", "24-hour market volume"),
  );
  const liquidity = Number(decimal(raw.liquidityNum ?? raw.liquidity ?? "0", "market liquidity"));
  return {
    id: text(raw.id, "market ID", 100),
    conditionId: raw.conditionId.toLowerCase(),
    question: text(raw.question, "market question"),
    slug: text(raw.slug, "market slug", 512),
    category:
      typeof raw.category === "string" && raw.category.trim()
        ? raw.category.slice(0, 100)
        : "Markets",
    volume24h,
    liquidity,
    endDate,
    outcomes: labels.map((label, i) => ({
      label: text(label, "outcome label", 200),
...
export function normalizeMarkets(
  payload: unknown,
  options: { search?: boolean; conditionId?: string } = {},
): Market[] {
  let rows: unknown[];
  if (options.search) {
    ...
    if (result.events === undefined && record(result.pagination, "search pagination")) rows = [];
      ...
      rows = result.events.flatMap((value) => {
        ...
        if (!Array.isArray(event.markets) || event.markets.length > 500)
          throw new PublicDataError("The provider returned invalid event markets.");
    ...
    if (!Array.isArray(payload) || payload.length > 500)
      throw new PublicDataError("The provider returned invalid market results.");
    rows = payload;
  }
  ...
  for (const row of rows) {
    ...
    if (!market) continue;
    if (options.conditionId && market.conditionId !== options.conditionId.toLowerCase())
      throw new PublicDataError("The condition lookup returned a different market.");
    const previous = markets.get(market.conditionId);
    if (previous && JSON.stringify(previous) !== JSON.stringify(market))
      throw new PublicDataError("The provider returned conflicting copies of a market.");
...
function normalizeFee(value: unknown): OrderBook["fee"] {
  if (value === undefined || value === null)
    return { kind: "unknown", reason: "The market did not provide its fee schedule." };
  try {
...
function normalizeLevels(value: unknown, side: "bids" | "asks", tick: string): BookLevel[] {
  if (!Array.isArray(value) || value.length > 10_000)
    throw new PublicDataError(`The provider returned invalid ${side}.`);
  const prices = new Set<string>();
  const levels = Array.from(value).map((row) => {
    const raw = record(row, `${side} level`);
    const price = decimal(raw.price, `${side} price`, {
      stringOnly: true,
      positive: true,
      max: "1",
    });
    const size = decimal(raw.size, `${side} size`, { stringOnly: true, positive: true });
    if (price === "1" || !new D(price).mod(tick).isZero())
      throw new PublicDataError(`The ${side} price does not match the current tick size.`);
    if (prices.has(price))
      throw new PublicDataError(
        `The provider returned duplicate ${side} price levels. Refresh before planning.`,
      );
    prices.add(price);
    return { price, size };
  });
  levels.sort((a, b) =>
    side === "bids" ? new D(b.price).comparedTo(a.price) : new D(a.price).comparedTo(b.price),
  );
  return levels;
}
...
export function normalizeOrderBook(
  ...
  if (!isTokenId(expected.tokenId) || !isConditionId(expected.conditionId))
    throw new PublicDataError("A valid token and condition ID are required.", 400);
  const condition = expected.conditionId.toLowerCase();
  if (
    raw.asset_id !== expected.tokenId ||
    !isConditionId(raw.market) ||
    raw.market.toLowerCase() !== condition ||
    !isConditionId(meta.c) ||
    meta.c.toLowerCase() !== condition
  ) {
    throw new PublicDataError(
      "Order book and market identities do not match the selected outcome.",
    );
  }
  if (!Array.isArray(meta.t) || meta.t.length < 2 || meta.t.length > 100)
    throw new PublicDataError("Market token membership could not be verified.");
  ...
  if (new Set(tokens).size !== tokens.length || !tokens.includes(expected.tokenId))
    throw new PublicDataError("The selected token does not belong to the verified market.");
  if (typeof meta.ao !== "boolean")
    throw new PublicDataError("Whether this market accepts orders could not be verified.");
  ...
  if (!TICKS.has(tickSize) || tickSize !== metaTick || minOrderSize !== metaMin)
    throw new PublicDataError("Book and market trading constraints disagree. Please refresh.");
  const timestampMs = integer(raw.timestamp, "book timestamp");
  const fetchedAt = integer(expected.fetchedAt, "fetch timestamp");
  if (timestampMs > fetchedAt + 60_000)
    throw new PublicDataError("The provider returned an implausible future book timestamp.");
  const hash = text(raw.hash, "book hash", 128);
  if (!/^[\da-fA-F]{32,128}$/.test(hash))
    throw new PublicDataError("The provider returned an invalid book hash.");
  const bids = normalizeLevels(raw.bids, "bids", tickSize);
  const asks = normalizeLevels(raw.asks, "asks", tickSize);
  if (bids[0] && asks[0] && new D(bids[0].price).gte(asks[0].price))
    throw new PublicDataError("The order-book snapshot is crossed. Please refresh.");
  return {
    ...
    fetchedAt,
    hash,
    fee: normalizeFee(meta.fd),
    acceptingOrders: meta.ao,
  };
}
...
export function normalizePositions(payload: unknown, account: string): PositionsPage {
  if (!isAccountAddress(account))
    throw new PublicDataError("Enter a valid 0x account address.", 400);
  const envelope = record(payload, "positions response");
  if (!Array.isArray(envelope.data) || envelope.data.length > 100)
    throw new PublicDataError("The provider returned invalid position rows.");
  const pagination = record(envelope.pagination, "positions pagination");
  if (typeof pagination.has_more !== "boolean")
    throw new PublicDataError("Position pagination could not be verified.");
  const seen = new Set<string>();
  const positions: Position[] = [];
  for (const value of envelope.data) {
    const raw = record(value, "position");
    if (
      !isAccountAddress(raw.proxy_wallet) ||
      raw.proxy_wallet.toLowerCase() !== account.toLowerCase()
    )
      throw new PublicDataError("The returned positions do not match the requested account.");
    if (!isTokenId(raw.token_id) || !isConditionId(raw.condition_id))
      throw new PublicDataError("A position could not be bound to its market and token.");
    if (seen.has(raw.token_id))
      throw new PublicDataError("The provider returned duplicate positions.");
    ...
    if (typeof raw.redeemable !== "boolean")
      throw new PublicDataError("Position redemption state could not be verified.");
    const position: Position = {
      tokenId: raw.token_id,
      conditionId: raw.condition_id.toLowerCase(),
      title: text(raw.title, "position title"),
      outcome: text(raw.outcome, "position outcome", 200),
      size,
      currentPrice,
      value: currentValue,
      slug: text(raw.slug, "position slug", 512),
...
async function requestJson(
  ...
  if (url.origin !== ORIGINS[origin])
    throw new PublicDataError("Unsupported public data endpoint.", 400);
  ...
  try {
    ...
    if (!response.ok)
      throw new PublicDataError(
        `The market-data provider is unavailable (HTTP ${response.status}). Please retry.`,
      );
    const body = await response.text();
    if (body.length > MAX_BODY_CHARS)
      throw new PublicDataError("The provider response exceeded the supported size.");
    return parsePublicJson(body);
  } catch (error) {
    if (error instanceof PublicDataError) throw error;
    throw new PublicDataError(
      "Public market data could not be loaded within 12 seconds. Please retry.",
    );
  }
}

export async function getMarkets(
  options: { q?: string; conditionId?: string } = {},
  fetcher: Fetcher = globalThis.fetch,
): Promise<Market[]> {
  if (options.q !== undefined && !isSearchQuery(options.q))
    throw new PublicDataError(
      "Search must be at most 120 characters without control characters.",
      400,
    );
  if (options.conditionId !== undefined && !isConditionId(options.conditionId))
    throw new PublicDataError("Enter a valid market condition ID.", 400);
  const q = options.q?.trim() ?? "";
  if (q && options.conditionId)
    throw new PublicDataError("Use a search term or a condition ID, not both.", 400);
  if (options.conditionId) {
    return normalizeMarkets(
      await requestJson(
        "gamma",
        "/markets",
        { condition_ids: options.conditionId, limit: "2" },
        fetcher,
      ),
...
export async function getOrderBook(
  options: { tokenId: string; conditionId: string },
  fetcher: Fetcher = globalThis.fetch,
): Promise<OrderBook> {
  if (!isTokenId(options.tokenId) || !isConditionId(options.conditionId))
    throw new PublicDataError(
      "A valid numeric token ID and market condition ID are required.",
      400,
    );
  const [book, metadata] = await Promise.all([
    requestJson("clob", "/book", { token_id: options.tokenId }, fetcher).then((raw) => ({
      raw,
      fetchedAt: Date.now(),
    })),
...
export function publicDataError(error: unknown): { error: string; status: 400 | 502 } {
  return error instanceof PublicDataError
