...
export interface VenueOrder {
 id:string; assetId:string; tokenId?:string; makerAddress:string;
 side:string; orderType:string; sizeMatched:string; originalSize:string;
 status:string; associateTrades:string[];
}
export interface VenueMakerOrder {
 orderId:string; assetId:string; tokenId?:string; makerAddress:string;
 side:string; matchedAmount:string;
}
export interface VenueTrade {
 id:string; takerOrderId:string; assetId:string; tokenId?:string;
 makerAddress:string; side:string; traderSide:'TAKER'|'MAKER';
 size:string; status:string; transactionHash:string; makerOrders:VenueMakerOrder[];
}

const D=Decimal.clone({precision:80});
const state=(s:string)=>typeof s==='string'?s.toUpperCase().replace(/^TRADE_STATUS_/,''):'';
const address=(s:unknown)=>typeof s==='string'&&/^0x[0-9a-fA-F]{40}$/.test(s)?s.toLowerCase():null;
...
function asset(value:unknown):bigint {
 if(typeof value!=='string'||!(/^(?:0|[1-9]\d{0,77})$/.test(value)||/^0x[0-9a-fA-F]{64}$/.test(value)))
...
function checkAsset(value:{assetId:string;tokenId?:string},expected:bigint){
 if(asset(value.assetId)!==expected||(value.tokenId!==undefined&&asset(value.tokenId)!==expected))
...
function checkSeller(value:{makerAddress:string;side:string},expected:string){
 if(address(value.makerAddress)!==expected)throw new Error('The venue returned a different order account.');
...
export function reconcileOrder(previous:TrackedOrder,order:VenueOrder,trades:VenueTrade[]):TrackedOrder{
 if(!id(order.id)||order.id!==previous.id)throw new Error('The venue returned a different order.');
 const account=address(previous.accountAddress);
 if(previous.mode!=='live'||!account)throw new Error('The saved live order account could not be verified.');
 const token=asset(previous.tokenId);
 checkAsset(order,token);checkSeller(order,account);
 ...
 if(matched.gt(requested))throw new Error('The venue reports more matched shares than requested.');
 ...
 if(!Array.isArray(trades)||trades.length>100_000)throw new Error('Trade reconciliation is incomplete.');

 const references=new Set(order.associateTrades);
 const related=new Map<string,{quantity:Decimal;status:string;hash:string;fingerprint:string}>();
 for(const trade of trades){
  ...
  if(!taker&&makerLegs.length===0){
   if(references.has(trade.id))throw new Error('A referenced trade belongs to a different order.');
   continue;
  }
  ...
  let amount:Decimal;
  if(taker){
   if(trade.traderSide!=='TAKER')throw new Error('The venue returned a different trade role.');
   checkAsset(trade,token);checkSeller(trade,account);
   amount=quantity(trade.size,'Trade shares',true);
  }else{
   if(trade.traderSide!=='MAKER')throw new Error('The venue returned a different trade role.');
   // Complementary matches may have a different token/side at the top level.
   const maker=makerLegs[0];checkAsset(maker,token);checkSeller(maker,account);
 ...
 if(settled.gt(matched))throw new Error('Confirmed fills exceed the order’s reported matched shares. Refresh again.');
 if(previous.settledShares!==null&&settled.lt(quantity(previous.settledShares,'Saved settled shares')))
  throw new Error('Confirmed fill history is incomplete or changed. Verify the order on Polymarket.');
 ...
 let status:TrackedOrder['status']='unknown',detail='The venue has not reported a final order state.';
 if(pending){status='settling';detail='Matched fills are awaiting venue settlement confirmation.'}
 else if(hasLiveRemainder){
  status='open';detail=settled.gt(0)
  ...
  const terminalState=['MATCHED','FILLED','CANCELED','CANCELLED','EXPIRED'].includes(state(order.status));
  if(settled.eq(requested)){status='settled';detail='All requested shares have venue-confirmed fills.'}
  else if(terminalState){status='partial';detail='Confirmed partial fill. The immediate order’s remaining shares were not sold.'}
  else{status='matched';detail='Some shares have confirmed fills; the remaining order state is not yet verified.'}
 }
 else if(failed&&allTradeIdsKnown&&allReturnedFinal){status='failed';detail='The venue reported a failed fill. Check the venue and remaining position before a new exit.'}
 else if(matched.gt(0)){status='matched';detail='The venue reports matched shares; complete settlement is not yet verified.'}
 else if(cancelled&&references.size===0&&fills.length===0){status='cancelled';detail='The venue confirmed cancellation. No matched shares were reported.'}
 else if(live){status='open';detail='The venue reports an open order. Refresh for matching or cancellation.'}
 return{...previous,matchedShares:matched.toFixed(),settledShares:settled.toFixed(),status,detail,netReceipt:null,
  transactionHashes:[...new Set(confirmed.map(t=>t.hash))],canCancel:hasLiveRemainder&&!pending};
}
