...
import {
  getMarkets, getOrderBook, getPositions,
  isAccountAddress, isConditionId, isSearchQuery, isTokenId,
  normalizeMarket, normalizeMarkets, normalizeOrderBook, normalizePositions,
  parsePublicJson, PublicDataError, publicDataError,
} from "./public-data";
...
function market(overrides = {}) {
  return {
    id: "123", conditionId: CONDITION, question: "Example event?", slug: "example-event",
    active: true, closed: false, archived: false, acceptingOrders: true, enableOrderBook: true,
    outcomes: '["Yes","No"]', clobTokenIds: JSON.stringify([TOKEN, OTHER_TOKEN]),
    outcomePrices: '["0.6","0.4"]', volume24hr: "12345.67", liquidity: "5000", endDate: "2026-09-16T00:00:00Z",
    ...overrides,
  };
}
...
function book(overrides = {}) {
  return {
    asset_id: TOKEN, market: CONDITION, timestamp: String(NOW - 1_000_000), hash: "a".repeat(40),
    bids: [{ price: "0.50", size: "10.25" }, { price: "0.60", size: "20" }],
    asks: [{ price: "0.80", size: "30" }, { price: "0.70", size: "40" }],
    tick_size: "0.01", min_order_size: "5", ...overrides,
  };
}

function metadata(overrides = {}) {
  return {
    c: CONDITION, t: [{ t: TOKEN, o: "Yes" }, { t: OTHER_TOKEN, o: "No" }],
    mos: 5, mts: 0.01, ao: true, fd: { r: 0.05, e: 1, to: true }, ...overrides,
  };
}
...
function position(overrides = {}) {
  return {
    token_id: TOKEN, condition_id: CONDITION, proxy_wallet: ACCOUNT,
    current_size: "12.345678", current_price: "0.6", current_value: "7.4074068",
    title: "Example event?", outcome: "Yes", slug: "example-event", redeemable: false, ...overrides,
  };
}

function positionsEnvelope(data = [position()], hasMore = false) {
  return { data, pagination: { limit: 100, offset: 0, has_more: hasMore, next_cursor: hasMore ? "opaque" : null } };
}

function fetchResponse(body: unknown, status = 200) {
  return vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json" } }));
}
...
describe("public request validation", () => {
  it("accepts bounded identifiers and rejects URL/path injection or unsafe uint256 values", () => {
    ...
    for (const input of ["https://evil.test", "../x", "1&other=2", "01", "1e10", "9".repeat(79), 123]) expect(isTokenId(input)).toBe(false);
  ...
  it("preserves numeric source precision rather than silently passing through binary rounding", () => {
    expect(parsePublicJson('{"size":0.123456789012345678,"big":999999999999999999,"small":1e-6,"flag":true}')).toEqual({
      size: "0.123456789012345678", big: "999999999999999999", small: "0.000001", flag: true,
    });
...
describe("market normalization", () => {
  it("binds labels to tokens and treats an indicative outcome price as nullable", () => {
    ...
    expect(value?.outcomes).toEqual([
      { label: "Yes", tokenId: TOKEN, price: "0.6" }, { label: "No", tokenId: OTHER_TOKEN, price: "0.4" },
    ]);
    expect(normalizeMarket(market({ outcomePrices: null }))?.outcomes[0].price).toBeNull();
  });

  it.each([{ active: false }, { closed: true }, { acceptingOrders: false }, { archived: true }, { enableOrderBook: false }])("filters non-tradeable markets %j", (overrides) => {
    expect(normalizeMarket(market(overrides))).toBeNull();
  });

  it.each([
    { conditionId: "bad" }, { outcomes: '["Yes"]' }, { outcomes: "broken json" },
    { clobTokenIds: JSON.stringify([TOKEN, TOKEN]) }, { outcomePrices: '["0.2"]' },
    { outcomePrices: '["1.1","0"]' }, { endDate: "nonsense" }, { question: "" },
  ...
  it("flattens search events, filters closed markets, and deduplicates identical results", () => {
    const payload = { events: [{ markets: [market(), market({ active: false })] }, { markets: [market()] }] };
    ...
    expect(() => normalizeMarkets({ events: [{ markets: "bad" }] }, { search: true })).toThrow(PublicDataError);
  });

  it("rejects condition lookup mismatch and conflicting duplicate markets", () => {
    expect(() => normalizeMarkets([market()], { conditionId: OTHER_CONDITION })).toThrow(/different market/);
    expect(() => normalizeMarkets([market(), market({ question: "Other event?" })])).toThrow(/conflicting/);
  });
});
...
describe("verified order-book binding", () => {
  it("sorts a copied book, uses current fee metadata and preserves last-change versus fetch time", () => {
    const raw = book(); const before = JSON.stringify(raw);
  ...
  it.each([
    ...
    [book(), metadata({ t: [{ t: OTHER_TOKEN, o: "No" }, { t: "123", o: "Other" }] })],
    [book(), metadata({ t: [{ t: TOKEN, o: "Yes" }, { t: TOKEN, o: "No" }] })],
  ...
  it.each([
    { bids: [{ price: "0.5", size: "1" }, { price: "0.50", size: "2" }] },
    ...
    { timestamp: "bad" }, { timestamp: String(NOW + 60_001) }, { hash: "not a hash" },
  ])("rejects corrupt depth or integrity metadata %j", (overrides) => {
    expect(() => normalizeOrderBook(book(overrides), metadata(), expected)).toThrow(PublicDataError);
  });

  it("rejects changing/inconsistent tick and order constraints instead of rounding them", () => {
    expect(() => normalizeOrderBook(book(), metadata({ mts: "0.001" }), expected)).toThrow(/constraints disagree/);
    expect(() => normalizeOrderBook(book(), metadata({ mos: "10" }), expected)).toThrow(/constraints disagree/);
    expect(() => normalizeOrderBook(book(), metadata({ ao: "true" }), expected)).toThrow(/accepts orders/);
  });

  it("allows a verified nonaccepting market for display without silently opening it", () => {
    expect(normalizeOrderBook(book(), metadata({ ao: false }), expected).acceptingOrders).toBe(false);
  });

  it.each([undefined, null, {}, { r: "bad", e: 1 }, { r: "0.05" }, { r: "0.05", e: 9 }])("keeps missing or malformed fees unknown: %j", (fd) => {
  ...
  it("accepts explicit zero fees without guessing from legacy base-fee fields", () => {
    expect(normalizeOrderBook(book(), metadata({ fd: { r: 0 } }), expected).fee).toEqual({ kind: "none" });
    expect(normalizeOrderBook(book(), metadata({ fd: undefined, tbf: 0 }), expected).fee.kind).toBe("unknown");
  });
});
...
describe("current v2 position normalization", () => {
  it("maps exact snake-case economics and reports pagination without claiming all positions", () => {
    expect(normalizePositions(positionsEnvelope([position()], true), ACCOUNT)).toEqual({
      positions: [{ tokenId: TOKEN, conditionId: CONDITION, title: "Example event?", outcome: "Yes", size: "12.345678", currentPrice: "0.6", value: "7.4074068", slug: "example-event", redeemable: false }],
      hasMore: true,
    });
    expect(normalizePositions(positionsEnvelope([]), ACCOUNT)).toEqual({ positions: [], hasMore: false });
  });

  it("applies the documented dust threshold locally and retains redeemed-price endpoints", () => {
    expect(normalizePositions(positionsEnvelope([position({ current_size: "0.0009" })]), ACCOUNT).positions).toEqual([]);
    expect(normalizePositions(positionsEnvelope([position({ current_size: "0.001", current_price: "1", redeemable: true })]), ACCOUNT).positions[0].redeemable).toBe(true);
  });

  it.each([
    { proxy_wallet: `0x${"d".repeat(40)}` }, { token_id: "bad" }, { condition_id: "bad" },
    { current_size: "-1" }, { current_price: "1.1" }, { current_value: null }, { redeemable: "false" },
  ])("rejects wrong account and malformed balances %j", (overrides) => {
    expect(() => normalizePositions(positionsEnvelope([position(overrides)]), ACCOUNT)).toThrow(PublicDataError);
  });
  ...
  it("rejects legacy payloads, missing pagination and duplicate positions", () => {
    ...
    expect(() => normalizePositions(positionsEnvelope([position(), position()]), ACCOUNT)).toThrow(/duplicate/);
  });
});
...
describe("fixed public network endpoints and route errors", () => {
  ...
  it("uses the requested fixed discovery and current position filters", async () => {
    const markets = fetchResponse([market()]); await getMarkets({}, markets);
    ...
    const positions = fetchResponse(positionsEnvelope([])); await getPositions(ACCOUNT, positions);
  ...
  it("fetches and cross-checks book and metadata before returning a book", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>().mockImplementation(async (url) => new Response(JSON.stringify(new URL(String(url)).pathname === "/book" ? book() : metadata())));
  ...
  it("rejects invalid inputs before fetching", async () => {
    ...
    await expect(getMarkets({ q: "x".repeat(121) }, fetcher)).rejects.toMatchObject({ status: 400 });
    await expect(getMarkets({ q: "yes", conditionId: CONDITION }, fetcher)).rejects.toMatchObject({ status: 400 });
    await expect(getOrderBook({ tokenId: "https://evil.test", conditionId: CONDITION }, fetcher)).rejects.toMatchObject({ status: 400 });
  ...
  it("turns upstream HTTP/network failures into friendly errors, never example data", async () => {
    await expect(getMarkets({}, fetchResponse({ error: "private upstream details" }, 503))).rejects.toMatchObject({ status: 502 });
    ...
    expect(publicDataError(new Error("secret internal stack"))).toEqual({ error: "Public market data is unavailable. Please retry.", status: 502 });
  });
  ...
  it("serves route JSON and validates missing/duplicate query parameters", async () => {
    ...
    expect((await positionsRoute(new Request("https://closeout.test/api/positions?account=bad"))).status).toBe(400);
    expect((await marketsRoute(new Request("https://closeout.test/api/markets?q=a&q=b"))).status).toBe(400);
    ...
    expect(await failure.json()).toEqual({ error: "The market-data provider is unavailable (HTTP 503). Please retry." });
  });
});
